...
<div class="card-body">
	...
	if($cari_barang){
		if($cari != ""){
			$sql = mysqli_query($koneksi,"select *from tb_tenant where unit_tn like '%$cari%' or nama_tn like '%$cari%'");
		}else{
			$sql = mysqli_query($koneksi,"select *from tb_tenant");
		}
	}else{
		$sql = mysqli_query($koneksi,"select *from tb_tenant LIMIT $posisi, $batas");
	}
...
<div style="margin-top: 10px;float: left;">
        <?php 
        $jml = mysqli_num_rows(mysqli_query($koneksi,"select *from tb_tenant"));
