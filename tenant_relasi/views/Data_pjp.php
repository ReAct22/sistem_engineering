...
<div class="card-body">
	...
	if($cari_barang){
		if($cari != ""){
			$sql = mysqli_query($koneksi,"select *from d_pjp where no_unit like '%$cari%' or nama_pjp like '%$cari%'");
		}else{
			$sql = mysqli_query($koneksi,"select *from d_pjp");
		}
	}else{
		$sql = mysqli_query($koneksi,"select *from d_pjp LIMIT $posisi, $batas");
	}
 ...
 ?>
	...
	<div style="margin-top: 10px;float: left;">
        <?php 
        $jml = mysqli_num_rows(mysqli_query($koneksi,"select *from d_pjp"));
