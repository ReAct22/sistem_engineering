...
<div style="margin-left: 30px;">
	...
	<a href="print_data/print_pl.php" class="w3bar-item w3-button w3-blue"><i class="fas fa-print"></i>Print</a>
	...
	if($cari_barang){
		if($cari != ""){
			$sql = mysqli_query($koneksi,"select *from pm_pl where id_pl like '%$cari%' or nama_utilitas like '%$cari%' or lokasi_utilitas like '%$cari%'");
		}else{
			$sql = mysqli_query($koneksi,"select *from pm_pl");
		}
	}else{
		$sql = mysqli_query($koneksi,"select *from pm_pl LIMIT $posisi, $batas");
	}
	...
	if($cek < 1){
		...
		<tr>
			...
			<td>
				...
				<a href="?page=delete_pl&id=<?php echo $data['id_pl']; ?>" class="btn btn-sm btn-danger"><i class="fas fa-trash"></i>Hapus</a>
			</td>
		</tr>
		<?php 
			}
        ...
        <?php 
        $jml = mysqli_num_rows(mysqli_query($koneksi,"select *from pm_pl"));
      ...
      <div style="margin-top: 10px; float: right;">
        <?php 
          ...
          for($i=1; $i<=$jml_hal; $i++){
        ?>
        <a href="?page=pm_pl&hal=<?php echo $i; ?>" class="btn btn-outline-primary"><?php echo $i; ?></a>
        <?php
          }
        ?>
      </div>
