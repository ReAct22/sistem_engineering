<?php 
	$query = mysqli_query($koneksi,"select max(id_gt) as kodePML from pm_gt");
		...
		$huruf = "PGT";//untuk membuat kode otomatis
...
<div class="card-body">
	...
	if($tambah_data){
		if($no_pm == "" || $periode == "" || $nama_utilitas == "" || $lokasi_utilitas == "" || $rencana == "" || $realisasi == "" || $nama_teknisi == "" || $jenis_kerjaan == "" ||  $nama_gambar == "" || $rekomendasi == "" ){
			...
			if($pindah){
				mysqli_query($koneksi,"insert into pm_gt values('$no_pm','$periode','$nama_utilitas','$lokasi_utilitas','$rencana','$realisasi','$nama_teknisi','$jenis_kerjaan','$nama_gambar','$rekomendasi')");
				?>
				<script type="text/javascript">
