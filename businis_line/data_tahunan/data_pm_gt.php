...
<div style="margin-left: 30px;">
	...
	<h1 style="margin-left: 30px;">PREVENTIF MAINTENANCE GROUND TANK</h1>
	<a href="?page=tambah_gt" class="w3-bar-item w3-button w3-green"><i class="fas fa-plus"></i>Tambah Data</a>
	<a href="print_data/print_gt.php" class="w3bar-item w3-button w3-blue"><i class="fas fa-print"></i>Print</a>
	...
	if($cari_barang){
		if($cari != ""){
			$sql = mysqli_query($koneksi,"select *from pm_gt where id_gt like '%$cari%' or nama_utilitas like '%$cari%' or lokasi_utilitas like '%$cari%'");
		}else{
			$sql = mysqli_query($koneksi,"select *from pm_gt");
		}
	}else{
		$sql = mysqli_query($koneksi,"select *from pm_gt LIMIT $posisi, $batas");
	}
	...
	if($cek < 1){
		...
		<tr>
			...
			<td>
				<a href="?page=edit_gt&id=<?php echo $data['id_gt']; ?>" class="btn btn-sm btn-primary"><i class="fas fa-edit"></i>Edit</a>
				<a href="?page=delete_gt&id=<?php echo $data['id_gt']; ?>" class="btn btn-sm btn-danger"><i class="fas fa-trash"></i>Hapus</a>
			</td>
		</tr>
		<?php 
			}
        ...
        <?php 
        $jml = mysqli_num_rows(mysqli_query($koneksi,"select *from pm_gt"));
      ...
      <div style="margin-top: 10px; float: right;">
        <?php 
          ...
          for($i=1; $i<=$jml_hal; $i++){
        ?>
        <a href="?page=pm_gt&hal=<?php echo $i; ?>" class="btn btn-outline-primary"><?php echo $i; ?></a>
        <?php
          }
        ?>
      </div>
