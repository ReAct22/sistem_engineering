<?php 
	$query = mysqli_query($koneksi,"select max(id_infrared) as kodePML from pm_infrared");
		...
		$huruf = "KPI";//untuk membuat kode otomatis
		$kodeData = $huruf.sprintf("%03s", $urutan);
?>
<div class="card-body">
	<h2>Input data PANEL TM</h2>
	...
	if($tambah_data){
		if($no_pm == "" || $periode == "" || $nama_utilitas == "" || $lokasi_utilitas == "" || $rencana == "" || $realisasi == "" || $nama_vendor == "" || $alamat_vendor == "" || $nama_teknisi == "" || $jenis_kerjaan == "" ||  $nama_gambar == "" || $rekomendasi == "" ){
			...
			if($pindah){
				mysqli_query($koneksi,"insert into pm_infrared values('$no_pm','$periode','$nama_utilitas','$lokasi_utilitas','$rencana','$realisasi','$nama_vendor','$alamat_vendor','$nama_teknisi','$jenis_kerjaan','$nama_gambar','$rekomendasi')");
				?>
				<script type="text/javascript">
