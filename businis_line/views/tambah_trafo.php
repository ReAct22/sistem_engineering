...
$query = mysqli_query($koneksi,"select max(id_trafo) as kodeTrafo from du_trafo");
		$data = mysqli_fetch_array($query);
		$kodeTrafo = $data['kodeTrafo'];

		$urutan = (int) substr($kodeTrafo, 3, 3);
	  ...
	  <span class="input-group-text">Keterangan</span>
		...
		if($tambah_data){
			if($no == "" || $nama_utili == "" || $lokasi == "" || $jumlah == "" || $keterangan == ""){
				...
				mysqli_query($koneksi,"insert into du_trafo values('$no','$nama_utili','$lokasi','$jumlah', '$keterangan')");
				?>
