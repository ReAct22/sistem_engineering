...
<br>
<a href="?page=tambah_build" class="btn btn-md btn-success ml-3"><i class="fas fa-plus"></i> Compline</a>
...
<div class="card-body">
	...
	if($cari_barang){
		if($cari != ""){
			$sql = mysqli_query($koneksi,"select *from db_building where nama like '%$cari%' or nik like '%$cari%'");
		}else{
			$sql = mysqli_query($koneksi,"select *from db_building");
		}
	}else{
		$sql = mysqli_query($koneksi,"select *from db_building LIMIT $posisi, $batas");
	}
	...
	<div style="margin-top: 10px;float: left;">
        <?php 
        $jml = mysqli_num_rows(mysqli_query($koneksi,"select *from db_building"));
      ...
      <div style="margin-top: 10px; float: right;">
        <?php 
          ...
          for($i=1; $i<=$jml_hal; $i++){
        ?>
        <a href="?page=data_build&hal=<?php echo $i; ?>" class="btn btn-outline-primary"><?php echo $i; ?></a>
        <?php
          }
        ?>
      </div>
