<?php 
	...
	$idcom = @$_GET['id'];
	$sql = mysqli_query($koneksi,"select *from co_sipil where id_complaine = '$idcom'");
	$data = mysqli_fetch_array($sql);
?>
<h3 class="ml-3">Update Compline Sipil</h3>
<form action="" method="post">
	<div class="card-body">
	  ...
	  <span class="input-group-text">NO.Complaine</span>
		...
		<input type="text" name="id" class="form-control" disabled="disabled" value="<?php echo $data['id_complaine'] ?>">
	</div>
	...
	<div class="input-group mb-3">
	  ...
	  <span class="input-group-text">Nama Tenant</span>
		...
		<input type="text" name="nama_tenant" class="form-control" value="<?php echo $data['nama_tenant'] ?>">
	</div>
	...
	<div class="input-group mb-3">
	  ...
	  <span class="input-group-text">Complaine</span>
		...
		<input type="text" name="komplaine" class="form-control" value="<?php echo $data['complaine'] ?>">
		</div>
...
if($tambah_data){
		mysqli_query($koneksi, "update co_sipil set nama_tenant = '$nama_tenant',
			tower = '$tower',
			lantai = '$lantai',
			unit = '$unit',
			complaine = '$kompline',
