...
$query = mysqli_query($koneksi,"select max(id_lift) as kodeLift from du_lift");
		$data = mysqli_fetch_array($query);
		$kodeLift = $data['kodeLift'];

		$urutan = (int) substr($kodeLift, 3, 3);
	  ...
	  <span class="input-group-text">Keterangan</span>
		...
		if($tambah_data){
			if($no == "" || $nama_utili == "" || $lokasi == "" || $jumlah == "" || $keterangan == ""){
				...
				mysqli_query($koneksi,"insert into du_lift values('$no','$nama_utili','$lokasi','$jumlah', '$keterangan')");
				?>
