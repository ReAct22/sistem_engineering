<?php 
	$query = mysqli_query($koneksi,"select max(id_tu) as kodePML from pm_tu");
...
<div class="card-body">
	...
	if($tambah_data){
		if($no_pm == "" || $periode == "" || $nama_utilitas == "" || $lokasi_utilitas == "" || $rencana == "" || $realisasi == "" || $nama_vendor == "" || $alamat_vendor == "" || $nama_teknisi == "" || $jenis_kerjaan == "" ||  $nama_gambar == "" || $rekomendasi == "" ){
			...
			if($pindah){
				mysqli_query($koneksi,"insert into pm_tu values('$no_pm','$periode','$nama_utilitas','$lokasi_utilitas','$rencana','$realisasi','$nama_vendor','$alamat_vendor','$nama_teknisi','$jenis_kerjaan','$nama_gambar','$rekomendasi')");
				?>
				<script type="text/javascript">
